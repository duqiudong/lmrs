...
$(function() {
	...
	$.ajax({
		type : "post",
		url : ADMINMAIN+"/upgrade/isneedtoupgrade",
...
function submitPassword() {
	...
	$.ajax({
		url : ADMINMAIN+"/login/modifypassword",
		type : "post",
		data : {
			"old_pass" : $("#pwd0").val(),
			"new_pass" : $("#pwd1").val()
...
function delcache() {
	$.ajax({
		url : ADMINMAIN+"/system/deletecache",
		type : "post",
		data : {},
		dataType : "json",
		success : function(data) {
...
function search() {
	...
	window.location.href = ADMINMAIN+"/goods/goodslist?search_info=" + search_info;
}
