...
$(function() {
	 ...
	 */
	// 临时隐藏面包屑和选项卡，后期会对后台首页进行整体界面修改 2016年11月15日 09:32:21 Pack老师
	// 销售统计
	onloadOrderChart(1);
...
function onloadUserInfo(){
	$.ajax({
		type : "post",
		url : admin_main+"/Index/getUserInfo",
		async : true,
		success : function(data) {
			...
			$(".last-login-time span").html(data['last_login_time']);
			$(".last-login-ip span").html(data['last_login_ip']);
		}
	});
}
/**
 * 加载会员提现审核中的数量
 * 2017年7月10日 12:12:51 Pack老师
 */
function onloadMemberBalanceWithdrawCount(){
	$.ajax({
		type : 'post',
		url : admin_main + "/index/getMemberBalanceWithdrawCount",
		success : function(data){
...
function onloadGoodsCount(){
	$.ajax({
		type : "post",
		url : admin_main+"/index/getgoodscount",
		success : function(data) {
			if(data['all']!=null){
...
function onloadOrderCount(){
	$.ajax({
		type : "post",
		url : admin_main+"/index/getordercount",
		success : function(data) {
...
function onloadSalesStatistics(){
	$.ajax({
		type : "post",
		url : admin_main+"/index/getsalesstatistics",
		success : function(data) {
			$(".yesterday_goods").html(data['yesterday_goods']);
			$(".month_goods").html(data['month_goods']);
			$(".yesterday_money").html(data['yesterday_money']);
...
function onloadOrderChart(e) {
	...
	$.ajax({
		type : "post",
		url : admin_main+"/index/getorderchartcount",
		data : {'date':e},
		success : function(data) {
...
function onloadConsultCount(){
	$.ajax({
		type : "post",
		url : admin_main+"/index/getconsultcount",
		success : function(data) {
...
function onloadWeiXinFansChart(e) {
	...
	$.ajax({
		type : "post",
		url : admin_main+"/index/getweixinfanschartcount",
		data : {'date':e},
		async : true,
		success : function(data) {	
			var timeOut = "关注人数";
