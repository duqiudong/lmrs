...
//		if(isLogin()){
			...
			cart_num = new Array();
			$.ajax({
				url : shop_main+"/goods/getshoppingcart",
				type : "POST",
				success : function(data){
					...
					if(data.length>0){//没登录会返回首页html代码
						for(var i=0;i<data.length;i++){
							...
							str += '<a href="'+shop_main+'/goods/goodsinfo.html?goodsid='+data[i].goods_id+'">';
							if(data[i]["picture_info"] != null){
								str += '<img src="'+upload+'/'+data[i]["picture_info"]["pic_cover_big"]+'" width="50" height="50" alt="'+data[i].goods_name+'"></a></span>';
							}else{						
								str += '<img src="'+TEMP_IMG+'/goods/default_goods_img.png" width="50" height="50" alt="'+data[i].goods_name+'"></a></span>';
							}
							str += '<div class="p-name">';
							str += '<a href="'+shop_main+'/goods/goodsinfo.html?goodsid='+data[i].goods_id+'" title="'+data[i].goods_name+'">'+data[i].goods_name+'&nbsp;'+data[i].sku_name+'</a></div>';
...
//		}
	...
	function refreshShopOrGoodsCollections(type){
		...
		if(isLogin()){
			$.ajax({
				url : shop_main+"/member/queryshoporgoodscollections",
				type : "POST",
				data : {"type" : type},
					...
					if(res.length>0){
						for(var i=0;i<res.length;i++){
							if(type == "shop"){
								...
								str += '<a href="'+shop_main+'/shop/shopindex?shop_id='+res[i].shop_id+'">';
								str += '<img src="'+upload+'/'+res[i].shop_avatar+'" width="50" height="50" alt="'+res[i].shop_name+'"></a></span>';
								
								str += '<div class="p-name">';
								str += '<a href="'+shop_main+'/shop/shopindex?shop_id='+res[i].shop_id+'">'+res[i].shop_name+'</a></div>';
								...
								str += '<div class="cart-item"><div class="item-goods"><span class="p-img">';
								str += '<a href="'+shop_main+'/goods/goodsinfo?goodsid='+res[i].goods_id+'">';
								str += '<img src="'+upload+'/'+res[i].pic_cover_mid+'" width="50" height="50" alt="'+res[i].goods_name+'"></a></span>';
								
								str += '<div class="p-name">';
								str += '<a href="'+shop_main+'/goods/goodsinfo?goodsid='+res[i].goods_id+'">'+res[i].goods_name+'</a></div>';
	...
	function refreshHistory(){
		...
		if(isLogin()){
			$.ajax({
				url : shop_main+"/goods/getmemberhistories",
				type : "POST",
				success : function(res){
					var str = "";
					if(res.length>0){
						for(var i=0;i<res.length;i++){
							str += '<div class="cart-item"><div class="item-goods"><span class="p-img">';
							str += '<a href="'+shop_main+'/goods/goodsinfo?goodsid='+res[i].goods_id+'">';
							str += '<img src="'+upload+'/'+res[i]["picture_info"].pic_cover_mid+'" width="50" height="50" alt="'+res[i].goods_name+'"></a></span>';
...
//				$.msg("请选择商品规格");
				$(".js-skulist").css("border","2px solid #c00");
...
//			alert(cart_num[i]+","+$("#hidden_max_buy").val());
	...
	function deleteShoppingCartById(id,flag)
	{
		if (confirm("您确实要把该商品移出购物车吗？")){
			$.ajax({
				url : shop_main+"/goods/deleteshoppingcartbyid",
				type : "POST",
