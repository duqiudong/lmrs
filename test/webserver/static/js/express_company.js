...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : ADMINMAIN+"/express/expresscompany",
		data : {
			...
			if (data["data"].length > 0) {
				for (var i = 0; i < data["data"].length; i++) {
					...
					if(data["data"][i]["express_logo"] != null &&data["data"][i]["express_logo"] !=''){
						html += '<td class="tal"><img src="'+UPLOAD+'/'+ data["data"][i]["express_logo"] +'" style="margin-right:10px;max-width:60px;max-height:60px;"/>' + data["data"][i]["company_name"] + '</td>';
					}else{
						...
						html += '<td><a href="expresstemplate?co_id='+ data["data"][i]["co_id"]+ '">打印模板</a>&nbsp;&nbsp;';
							html += '<a href="updateexpresscompany?co_id='+ data["data"][i]["co_id"]+ '">修改</a><br/>';
							html += '<a href="'+ADMINMAIN+'/express/freighttemplatelist?co_id='+ data["data"][i]["co_id"]+ '">运费模板</a>&nbsp;&nbsp;';
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url : ADMINMAIN+"/express/expresscompanydelete",
					data : {
